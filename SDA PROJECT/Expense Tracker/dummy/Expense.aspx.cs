...
namespace dummy
{
    public partial class Expense1 : System.Web.UI.Page
    {
        ...
        protected void NewEX(object sender, EventArgs e)
        {
            Response.Redirect("Transaction1.aspx");
        }
        protected void Del_EXP(object sender, GridViewDeleteEventArgs e)
        {
            Label id = GD.Rows[e.RowIndex].FindControl("lbl_ID") as Label;
            b.DeleteExpense(Convert.ToInt32(id.Text));
            Response.Redirect("Expense.aspx");
        }

        protected void Cancel_Up_EXP(object sender, GridViewCancelEditEventArgs e)
        {
            GD.EditIndex = -1;
            View();
        ...
        protected void Edit_Expense(object sender, GridViewEditEventArgs e)
        {
            GD.EditIndex = e.NewEditIndex;
            View();
        }
        protected void Up_Expense(object sender, GridViewUpdateEventArgs e)
        {
            try
            {
                Label id = GD.Rows[e.RowIndex].FindControl("lbl_ID") as Label;
                ...
                conn.Open();
                SqlCommand cmd = new SqlCommand("Update EXPEN set Name=@Name, Amount=@Amount, Description=@Description where id=@Id", conn);
        ...
        public void View()
        {
            GD.DataSource = b.ViewExpense(Login.user);
            GD.DataBind();
        }
        ...
        protected void Filter(Object sender, EventArgs e)
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string queryInc = "SELECT * FROM EXPEN WHERE Category = @Category";
                    using (SqlCommand cmd = new SqlCommand(queryInc, conn))
                    {
                        cmd.Parameters.AddWithValue("@Category", ecategory.Value);
        ...
        protected void LoadCategories()
        {
            ...
            try
            {
                using (conn)
                {
                    conn.Open();
                    DataTable dtInc = new DataTable();
                    string queryInc = "SELECT Category FROM EXP_Categories";
                    using (SqlCommand cmd = new SqlCommand(queryInc, conn))
                    {
                        using (SqlDataAdapter adapter = new SqlDataAdapter(cmd))
                        {
